...
# =========================================================
# ✅ SAMG 전용 (캐치티니핑) 서버
# - 기존 시셰이도 운영 서버와 "절대 충돌" 안 나게 분리
#   1) 환경변수: SAMG_ 프리픽스
#   2) DATA_DIR: data_samg (DB/MASTER 완전 분리)
#   3) S3 prefix: samg/products/ (키 충돌 방지)
#
# ✅ 사용자 제공 값 반영
#   - S3 bucket: samg-master (기본값)
#   - region: us-east-1 (기본값)
#   - public ipv4: 13.209.84.179 (기본값)
# =========================================================
...
# ✅ SAMG 기본값 (사용자 제공)
# -------------------------
DEFAULT_SAMG_S3_BUCKET = "samg-master"
DEFAULT_SAMG_AWS_REGION = "us-east-1"
DEFAULT_SAMG_PUBLIC_IPV4 = "13.209.84.179"
...
#   - 없으면 기본값 사용
# -------------------------
S3_BUCKET = os.environ.get("SAMG_S3_BUCKET", DEFAULT_SAMG_S3_BUCKET).strip()
AWS_REGION = os.environ.get("SAMG_AWS_REGION", DEFAULT_SAMG_AWS_REGION).strip()

AWS_ACCESS_KEY_ID = os.environ.get("SAMG_AWS_ACCESS_KEY_ID", "").strip()
AWS_SECRET_ACCESS_KEY = os.environ.get("SAMG_AWS_SECRET_ACCESS_KEY", "").strip()

PRESIGNED_EXPIRES = int(os.environ.get("SAMG_PRESIGNED_EXPIRES", "1800"))
PRESIGNED_REFRESH_MARGIN = int(os.environ.get("SAMG_PRESIGNED_REFRESH_MARGIN", "120"))

FLASK_SECRET_KEY = os.environ.get("SAMG_FLASK_SECRET_KEY", "change-this-to-a-random-secret")
AUTO_LOAD_MASTER = os.environ.get("SAMG_AUTO_LOAD_MASTER", "1").strip()

MAX_CONTENT_LENGTH = int(os.environ.get("SAMG_MAX_CONTENT_LENGTH", str(20 * 1024 * 1024)))

VIEW_PASSWORD = os.environ.get("SAMG_VIEW_PASSWORD", "").strip()
EDIT_PASSWORD = os.environ.get("SAMG_EDIT_PASSWORD", "").strip()

FLASK_DEBUG = os.environ.get("SAMG_FLASK_DEBUG", "0").strip() == "1"
COOKIE_SECURE = os.environ.get("SAMG_COOKIE_SECURE", "1").strip() == "1"

# ✅ SAMG 서버 퍼블릭 주소(선택)
# - 템플릿/문구/로그/리다이렉트 품질 개선용
# - 기본: http://13.209.84.179
PUBLIC_BASE_URL = os.environ.get("SAMG_PUBLIC_BASE_URL", f"http://{DEFAULT_SAMG_PUBLIC_IPV4}").strip()

# ✅ S3 Key Prefix (SAMG 전용)
S3_PREFIX = os.environ.get("SAMG_S3_PREFIX", "samg/products/").strip()
...
if not all([S3_BUCKET, AWS_REGION, AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY]):
    raise SystemExit(
        "필수 환경변수 누락(SAMG):\n"
        "- SAMG_S3_BUCKET (없으면 기본값 samg-master 사용)\n"
        "- SAMG_AWS_REGION (없으면 기본값 us-east-1 사용)\n"
        "- SAMG_AWS_ACCESS_KEY_ID\n"
        "- SAMG_AWS_SECRET_ACCESS_KEY\n"
        "\n서버 배포 시에도 환경변수로 반드시 설정하세요."
    )

if not VIEW_PASSWORD or not EDIT_PASSWORD:
    raise SystemExit(
        "필수 환경변수 누락(SAMG): SAMG_VIEW_PASSWORD, SAMG_EDIT_PASSWORD\n"
...
# =========================
# 기본 설정 (SAMG 전용 데이터 폴더)
# =========================
BASE_DIR = Path(__file__).resolve().parent

# ✅ 기존 시셰이도: data/
# ✅ SAMG 전용: data_samg/
DATA_DIR = BASE_DIR / "data_samg"
...
def healthz():
    return jsonify({
        ...
        "public_ipv4_default": DEFAULT_SAMG_PUBLIC_IPV4,
    })
...
def _validate_master_df(df: pd.DataFrame):
    ...
    if dup_codes:
        warnings.append(
            f"ITEM_CODE 중복 {len(dup_codes)}건 (샘플): {dup_codes[:20]}{'...' if len(dup_codes) > 20 else ''}"
        )
...
# =========================
def rebuild_photos_from_s3(prefix: str = None) -> dict:
    """
    S3의 samg/products/<item_code>/<filename> 를 스캔하여
    ...
    for page in paginator.paginate(Bucket=S3_BUCKET, Prefix=prefix):
        for obj in page.get("Contents", []):
            ...
            if "/" not in rest:
                continue
            item_code, filename = rest.split("/", 1)
...
def admin_master_upload_confirm():
    ...
    try:
        ...
        flash(
            f"마스터 동기화 완료(SAMG): 반영 {result['upserted']}건 / 비활성화 {result['deactivated']}건 (사진/S3 원본 유지)",
            "success"
        )
        return redirect(url_for("register_home"))
...
def view_export_products_xlsx():
    ...
    filename = f"SAMG_상품리스트_{ts}.xlsx"
...
def main_cli():
    ...
    if args.command == "rebuild_photos_from_s3":
        result = rebuild_photos_from_s3(prefix=S3_PREFIX)
